...
public sealed class PercentToOpacityConverter : IValueConverter
{
    public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        if (value is string s && double.TryParse(s, out var parsed)) return Math.Clamp(parsed / 100d, 0d, 1d);
    ...
    public object ConvertBack(object value, Type targetType, object parameter, CultureInfo culture)
    {
        ...
        if (value is string s && double.TryParse(s, out var parsed)) return Math.Clamp(parsed * 100d, 0d, 100d);
